...
class SOM:
    ...
    def __init__(self, inputsize, gridsize):
        ...
        # Carte des activités
        self.activitymap = []
        for posx in xrange(gridsize[0]):
            mline = []
            wmline = []
            amline = []
            for posy in xrange(gridsize[1]):
    ...
    def compute(self, x):
        ...
        for posx in xrange(self.gridsize[0]):
            for posy in xrange(self.gridsize[1]):
...
if __name__ == '__main__':
    ...
    # Ensemble de données personnalisées
    # On affiche un rectangle sur toute la partie haute
    nsamples = 1000
    samples1 = numpy.random.random((nsamples // 2, 2, 1))
    samples1[:, 0, :] -= 1
    samples2 = numpy.random.random((nsamples // 2, 2, 1))
